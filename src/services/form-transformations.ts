...
function transformHeadersToEndpoint(headersForm: HeadersForm): {
  ...
  return headersForm.reduce((object, item) => {
    if (!item.headerName || !item.headerValue) {
      return object;
    }
    return Object.assign(object, { [item.headerName]: item.headerValue });
  }, {});
}
function transformHeadersToForm(headersEndpoint: {
  [key: string]: string;
}): HeadersForm {
  return Object.entries(headersEndpoint).map(([headerName, headerValue]) => ({
    headerName,
    headerValue,
  }));
}
